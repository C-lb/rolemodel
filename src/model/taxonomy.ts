...
export const TAXONOMY: readonly LineItemDef[] = [
  ...
  { key: "revolver_movement", statement: "cashflow", label: "Revolver drawn (repaid)", order: 695, parentKey: "cash_from_financing", isSubtotal: false,
    definition: "The forecast plug's cash effect: positive when the revolver is drawn to defend the minimum-cash floor, negative when surplus cash repays it. Always absent in extracted historicals, since no historical document reports a forward-looking plug. It exists so the financing subtotal stays the sum of its own components rather than hiding the plug inside itself.",
    absentMeansZero: true },
